...
int executeLine(const std::string& current_line, VM& vm) {
    ...
    if (cmd == "mov") {
        ...
        if (tokens.size() >= 4) {
            int b = getValue(tokens[3], vm);
            if (b != 0) setValue(tokens[1], getValue(tokens[2], vm) % b, vm);
        }
    }
        ...
        if (tokens.size() >= 4) {
            int v1 = getValue(tokens[2], vm);
            int v2 = getValue(tokens[3], vm);
            setValue(tokens[1], (cmd == "and") ? (v1 && v2) : (v1 || v2), vm);
        }
    }
    else if (cmd == "rnd") {
        if (tokens.size() >= 4) {
            int min_v = getValue(tokens[2], vm);
            int max_v = getValue(tokens[3], vm);
            if (max_v > min_v) {
                int result = min_v + (rand() % (max_v - min_v + 1));
                setValue(tokens[1], result, vm);
            }
        }
    }
    else if (cmd == "inc" || cmd == "dec") {
        if (tokens.size() >= 2) {
            vm.variables[tokens[1]] += (cmd == "inc" ? 1 : -1);
        }
    }
    else if (cmd == "color") {
        if (tokens.size() >= 2) {
            printf("\x1b[%dm", getValue(tokens[1], vm));
        }
    }
    else if (cmd == "reset") {
        printf("\x1b[0m");
        ...
        if (tokens.size() >= 3) {
            Beep(getValue(tokens[1], vm), getValue(tokens[2], vm));
        }
    }
    else if (cmd == "title") {
        size_t pos = current_line.find("title") + 6;
        if (pos < current_line.length()) {
            SetConsoleTitleA(current_line.substr(pos).c_str());
        }
    }
    else if (cmd == "PXL") {
        if (tokens.size() >= 4)
